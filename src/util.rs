...
fn download_input(day: i32, path: &std::path::PathBuf) {
    ...
    let cookie_value = HeaderValue::from_str(&format!("session={}", session)).unwrap();
...
pub fn get_input(day: i32) -> String {
    let path = std::path::Path::new(PROJECT_ROOT).join("input").join(format!("day{day}.txt"));
